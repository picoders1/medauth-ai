...
| **R-51** | **A criterion may be unverifiable on its own.** 42 CFR 410.32 `(b)(3)` sets supervision levels that criterion C03 references but does not contain, so adjudicating C03 in isolation cannot reach a correct verdict. | **High** | **Located, not fixed.** The provision is now ranked **priority 1** in the OD-19 reviewer package - the urgent tier reserved for provisions a transcribed criterion may be unevaluable without - and 410.32's declared logic records it as UNRESOLVED rather than declaring a shape for C03. Fixing it means transcribing (b)(3), which is gated on qualified review. | **Open** | — |
...
| **R-59** | **The logic inventory records a doubt the runtime does not act on.** Four policy versions are classified REVIEW_REQUIRED because they carry exception wording with no declared logic, but at runtime they still receive an ASSUMED_CONJUNCTION. | **High** | Not mitigated. Acting on it means either declaring their logic (which requires OD-19 review) or routing them to HUMAN_REVIEW (which invalidates gold_v1 on the strength of a lexical scan). Neither was chosen; the gap is recorded as OD-24. | **Open** | — |
