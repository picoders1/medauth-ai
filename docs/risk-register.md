...
| **R-51** | **A criterion may be unverifiable on its own.** 42 CFR 410.32 `(b)(3)` sets supervision levels that criterion C03 references but does not contain, so adjudicating C03 in isolation cannot reach a correct verdict - the model would be asked a question the evidence set cannot answer. | **High** | Recorded as one of the seven confirmed gaps. Not mitigated: fixing it means transcribing the referenced provision, which is part of the review OD-19 gates. Until then C03's verdicts are not trustworthy at the level the citation contract implies. | **Open** | — |
| **R-52** | **The decision table assumes required criteria are conjunctive; at least one policy is not.** 42 CFR 410.32(a) carries a mammography exception - an alternative route to satisfying the ordering requirement. Applied universally, the table would deny or hold a case the regulation permits. | **High** | Not mitigated. No gold case currently exercises the exception, so it does not corrupt existing labels, but the logic is wrong in the direction that harms a patient. Tracked as OD-23; changing the table is a clinical behaviour change requiring an ADR-010 amendment. | **Open** | 4 |
