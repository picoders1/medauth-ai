...
| **OIDC provider integration is implemented** | Discovery, issuer-tied key retrieval, signature/issuer/audience/expiry validation, rotation | `pytest tests/security/test_reviewer_identity.py` (35) | **Permitted.** Provider-neutral: no vendor is named, and `jwks_uri` comes from the issuer's own discovery document |
| **Enterprise SSO is deployed** | A live identity provider configured, pointed at, and exercised | — | **Refused.** No provider is selected (**ADR-030**): none is named in any tracked file. No issuer has been pointed at a real provider and no deployment has been performed |
