...
| **The reviewer who decided a case is known** | Authenticated human identity, distinct from the API integrator | — | **Refused.** The API key identifies the integrating **caller**; a reviewer names themselves and nothing verifies it. `reviewer_id` records who *claimed* to decide. Tracked as **OD-43** |
