...
| **OIDC provider integration is implemented** | Discovery, issuer-tied key retrieval, signature/issuer/audience/expiry validation, rotation | `pytest tests/security/test_reviewer_identity.py` (37) | **Permitted.** Provider-neutral: no vendor is named, and `jwks_uri` comes from the issuer's own discovery document |
| **The authentication boundary has been exercised against a real provider** | A live OIDC provider issuing tokens this repository did not sign, driven through the application | `verify_idp.py` 12/12, `verify_idp_e2e.py` 39/39 (in-process), `verify_idp_container_e2e.py` **45/45 through the published port** against Keycloak 26, realm `medauth-nonprod` | **Permitted, non-production only** (ADR-030 amendment). Real RS256 tokens across the real container network; full authorization matrix, HITL, audit, 19 negative cases, a real key rotation, and a provider outage that failed closed. Three defects were found this way and none by the mocked suite. No script is in the suite - a live provider must not be a CI dependency |
| **The containerised deployment starts and serves the current code** | The image built from HEAD, running, answering on the published port | `docker compose up -d --build` then `verify_idp_container_e2e.py` | **Permitted.** It did not, until this phase: jinja2 was an undeclared runtime dependency and the image could not import the application. Guarded by `tests/unit/test_runtime_dependencies.py` |
| **The architecture works in a production-SHAPED topology** | HTTPS, a stable hostname, verified TLS, a non-development IdP on persistent storage, a clean image, driven end to end | `compose.prod-shape.yaml` + `verify_idp_container_e2e.py` **45/45** | **Permitted, and it is not a production claim.** Keycloak `start` (profile `prod`) on PostgreSQL with a named volume; realm, users and signing `kid` survive a restart; TLS verified against a local CA, with untrusted-CA and wrong-hostname connections both refused. Classified item by item in `docs/deployment/production-shape-contract.md` §8 |
| **A deployment can be smoke-tested without writing to the clinical record** | A suite that runs against a live deployment and provably writes nothing in its default mode | `scripts/production_smoke.py` — safe mode **7/7 with zero review-event rows written** (16 → 16), mutating mode **15/15**, against the local production-shaped stack | **Permitted.** Ten of the twelve contract checks are reads or denials and are safe anywhere; the two that finalise a case run only against an operator-designated `--smoke-case-id`. The script never creates a case and never mints a token. **It is a deployment check, not clinical validation** |
| **Production SSO / production deployment** | A production deployment, on an owned domain, with a real certificate authority, an operator and an approved policy | — | **Refused.** None of it exists. The production-shaped environment is one machine, a throwaway CA and four fixture identities |
| **Enterprise SSO is deployed** | A production identity provider, operated, with real accounts and a rotation policy | — | **Still refused.** A *non-production* Keycloak was stood up and verified (ADR-030 amendment, `docs/security/nonproduction-idp.md`), which is a different claim: `start-dev`, in-memory storage, four fixture identities, no HTTPS enforcement, no MFA, no provisioning, no rotation observed. No production provider is selected or deployed |
| **A candidate provider can be checked against the contract** | A conformance run against a real issuer | `uv run python scripts/verify_idp.py --issuer … --audience …` | **Permitted as a capability, not a result.** The verifier is tested against a simulated provider (8 tests); **no real provider has been checked**, because none exists to point at |
| **The `sub` claim is stable and never reassigned** | The provider operator's assurance | — | **Refused, and unverifiable by any script.** If a provider reassigns `sub`, an old decision becomes attributed to a new person. It must be asked of whoever operates the eventual provider |
...
| **Runs under Docker Compose** | A stack that starts and passes readiness | `docker compose up -d && curl :8015/ready` | **Produced** (Phase 0) — API and pgvector healthy; all four readiness checks pass; container non-root, read-only rootfs, all capabilities dropped. *The Phase 0 run used `:8010`, the published port at the time; the command above is the current one (ADR-019 amendment, 2026-08-26). The port moved; the evidence was not re-produced.* |
| **Runs on Kubernetes** | A real cluster run producing an artefact | — | **Refused.** No cluster exists on the reference machine (no `kubectl`/`kind`/`minikube`/`helm`). |
| **Kubernetes manifests are authored and statically validated in CI** | Manifest files, and a CI step that validates them | — | **Refused — and it was asserted here until 2026-08-26.** No manifest exists in this repository, and `kubeconform` appears in `ci.yaml` only in a comment describing future work. This was the *permitted, careful* version of the Kubernetes claim, which made it the more dangerous one: it read as the honest statement while being false. Corrected in the ADR-019 amendment |
