...
| **The authentication boundary has been exercised against a real provider** | A live OIDC provider issuing tokens this repository did not sign, driven through the application | `scripts/verify_idp.py` 12/12 and `scripts/verify_idp_e2e.py` 39/39 against Keycloak 26, realm `medauth-nonprod` | **Permitted, non-production only** (ADR-030 amendment). Real RS256 tokens; the full authorization matrix, HITL workflow, audit and 16 negative security cases. It found a defect the mocked tests could not: an unknown `kid` produced a 500, not a refusal. Neither script is in the suite - a live provider must not be a CI dependency |
