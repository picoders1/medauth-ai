...
|---|---|---|---|---|
| **OD-1** | Does the configured model support `response_format: json_schema`, `json_object`, or tool calling — and how does it behave on schema violation? | Unknown. It determines how closed schemas are enforced in practice, and every structured-output design choice downstream. | `scripts/probe_model_capabilities.py` report; ADR-008 amended | **0 — blocking** |
